...
대규모 엔터프라이즈 SaaS 플랫폼 위에서는 여러 인스턴스에 걸친 계정 생성/비활성화, 구축 비용 산정, 보안 취약점 대응 항목 생성처럼 매번 손으로 반복하던 작업을 스크립트와 저코드로 자동화해왔고, 독립적으로 진행한 풀스택 프로젝트에서도 강사 배정과 정산처럼 사람이 일일이 처리하던 업무를 시스템으로 옮기는 데 집중했습니다. 이 과정에서 고객사 실무자와 직접 요구사항을 협의하고 조율하며, 현장의 문제를 정확히 정의하는 것이 자동화 설계의 출발점이라는 것을 배웠습니다.
